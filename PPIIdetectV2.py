"""
Módulo de detección automática de hélices de tipo Poliprolina II (PPII) en proteínas usando PyMOL.

Este script proporciona una interfaz gráfica para:
- Cargar o descargar estructuras proteicas en formato PDB.
- Añadir hidrógenos y eliminar solventes.
- Calcular ángulos diedros φ y ψ para cada residuo.
- Detectar segmentos candidatos a hélices PPII.
- Identificar enlaces no canónicos tipo Cα–H···O.
- Exportar resultados en distintos formatos (.txt, .csv, .pdb).

Requisitos:
- PyMOL (probado con versión 3.1.4.1).
- Python 3.9 o superior.
- Interfaz Tkinter (incluida en la mayoría de distribuciones estándar de Python).

Autora: Silvia Enma Rodríguez Fernández
Proyecto de TFG – Ingeniería Informática – UNIR
"""
...
# Variables globales
pdb_file = None  # Ruta o ID de la proteína cargada
segmentos_ppii_global = []  # Lista de segmentos candidatos a hélice PPII
